...
public final class DisplayCostsMain {
    ...
    private void start() {
        ...
        // temporarily comment in the next line to display a representation of
        // the graph
        ...
        // add code here to set diameter correctly or write a helper method to
        // do it
        diameter = 0;
        for (int i = 0; i < myGraph.getNumberOfVertices(); i++) {
            for (int j = 0; j < myGraph.getNumberOfVertices(); j++) {

                if (myGraph.minimalPath(vertices[i], vertices[j]) > diameter) {
                    diameter = myGraph.minimalPath(vertices[i], vertices[j]);
                }
            }
        }
        ...
        while (runAgain) { // loops until the user chooses to quit
            ...
            from = promptForChoice(console,
                                   "\nChoose a city to start at " + "(1 - "
                                            + myGraph.getNumberOfVertices() + ") or enter "
                                            + QUIT_OPTION + " to quit the program : ");
            ...
            if (from == QUIT_OPTION) {
                ...
                for (int i = 0; i < myGraph.getNumberOfVertices(); i++) {
                    if (myGraph.getEdgeWeight(vertices[i], vertices[from - 1]) > 0) {
                        degree++;
                    }
                }
                System.out.println(vertices[from - 1] + " has degree " + degree + ".");

                // get the user's choice for an end city
                to = promptForChoice(console,
                                     "\nChoose a city to end at (1 - "
                                              + myGraph.getNumberOfVertices() + ") or "
                                              + QUIT_OPTION + " to quit : ");
            }
            if (to == QUIT_OPTION) {
                runAgain = false;
     ...
     * 
     * This method is adopted from getInt() on page 315 of "Building Java
     * Programs" by Reges and Stepp
     * 
     * @param theConsole a Scanner used to capture user input
     * @param thePrompt a prompt to the user
    ...
    private void displayPathLength(final Vertex<String> thePoint1,
        ...
        pathLength = myGraph.minimalPath(thePoint1, thePoint2);
